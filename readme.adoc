...
实验的内容会根据在K8S在AWS实践的情况进行定期更新。这个实验需要用到海外的AWS账号，以及一台笔记本。进行如下实验会产生对应的使用费用，如果使用免费级别，预计为每天20$左右，具体价格可以参考如下链接：https://calculator.s3.amazonaws.com/index.html#r=FRA&s=EC2&key=calc-E6DBD6F1-C45D-4827-93F8-D9B18C5994B0[K8S预估价格]
