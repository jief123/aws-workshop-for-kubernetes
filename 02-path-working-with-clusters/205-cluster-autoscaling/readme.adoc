...
This chapter uses a cluster with 3 master nodes and 3 worker nodes as described here: link:https://github.com/jief123/aws-workshop-for-kubernetes/tree/master/01-path-basics/102-your-first-cluster[multi-master, multi-node gossip based cluster].
...
If the value of `MaxSize` is 3 (which means you can't scale beyond 3 worker nodes) then update it using kops using the following command:
...
Change the value of `maxSize` to `4`. Save the configuration file and exit out of the editor.
...
Before you Create deployment, you need to add a cluster roles to default user.
...
You should see a result of 3, or whatever was the initial size of your cluster.
...
This correctly shows 4, as was set earlier in this chapter.
...
2. 更改集群IAM配置之后node role策略增加了哪些内容？

3. More please check https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler/cloudprovider/aws.
